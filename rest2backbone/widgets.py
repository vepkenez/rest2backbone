...
def mixin(instance, mixin_class):
    ...
    for name in mixin_class.__dict__:
        if name.startswith('__') and name.endswith('__') or not isinstance(mixin_class.__dict__[name], FunctionType):
            continue
...
class InputMixin(object):
    def render_template(self, name, attrs=None, field=None):
        ...
        # self.attrs = attrs  # TODO: looks like this line is required here; otherwise "self.attrs" is wrong bellow
...
class SelectMixin(object):
    ...
    def render_option(self, selected_choices, option_value, option_label):
        ...
        if self.allow_multiple_selected:
            js = """<%%
            if ( %(name)s && (%(name)s.indexOf("%(val)s")>-1|| %(name)s.indexOf(+"%(val)s")>-1))
                { print(" selected"); }
            %%>""" % {'name': self.name, 'val': val}
        else:
            js = '<%% if ( %s == "%s") { print(" selected")}%%>' % (self.name, val)
        return u'<option value="%s"%s>%s</option>' % (val, js, conditional_escape(force_unicode(option_label)))
...
class DummyMixin (object):
    def render_template(self, name, attrs=None, field=None):
        return mark_safe('<span id="%s">Widget for %s is not ready for templates</span>' %
                         (name, attrs.get('id') or ''))


#defines patches tuples of Original_Widget_Class,  Patch_Mixin_Class
PATCHES = [
    (widgets.Input, InputMixin),
...
class DynamicWidget(widgets.TextInput):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attrs['class'] = 'r2b_dynamic' if not 'class' in self.attrs else self.attrs['class'] + ' r2b_dynamic'
    ...
    def js_options(self, name, attrs, field):
        ...
        """
        return self.options if hasattr(self.options) else {}  # FIXME!
...
class DynamicSelect(DynamicRelatedWidget):
    ...
    def js_options(self, name, attrs, field):
        ...
        opts = {'indexModel':index_model_js, 'required': field.required}
...
class DynamicEditor(DynamicRelatedWidget):
    ...
    def render_template(self, name, attrs, field):
        ...
        output = list(['<div%s>' % flatatt(final_attrs)])
